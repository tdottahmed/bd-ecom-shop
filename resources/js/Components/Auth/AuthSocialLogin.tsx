...
export default function AuthSocialLogin({ type = 'Login' }: { type?: 'Login' | 'Signup' }) {
    const { socialProviders } = usePage().props as unknown as { socialProviders: SocialProviders };

    const googleEnabled   = socialProviders?.google?.enabled   ?? false;
    const facebookEnabled = socialProviders?.facebook?.enabled ?? false;
    ...
    return (
        <div className="mt-8">
            ...
            <div className={`mt-6 grid gap-4 ${googleEnabled && facebookEnabled ? 'grid-cols-2' : 'grid-cols-1'}`}>
                {googleEnabled && (
                    <a
